...
PRIVATE INODE * new_inode(int dev, int inode_nr, int start_sect);
PRIVATE void new_dir_entry(INODE * dir_inode, int inode_nr, char * filename);
...
PRIVATE INODE * create_file(char * path, int flags)
{
	...
	INODE *newino = new_inode(dir_inode->i_dev, inode_nr,
					 free_sect_nr);
	...
	for (i = 0; i < sb->nr_imap_sects; i++) {
		...
		for (j = 0; j < SECTOR_SIZE; j++) {
			...
			if (fsbuf[j] == 0xFF)
				continue;
			/* skip `1' bits */
			for (k = 0; ((fsbuf[j] >> k) & 1) != 0; k++) {} //这里忽略了内存空洞的情况，直接以为1的条件循环到了第一个不为1的地方
	...
	for (i = 0; i < sb->nr_smap_sects; i++) { /* smap_blk0_nr + i :
		...
		for (j = 0; j < SECTOR_SIZE && nr_sects_to_alloc > 0; j++) {
			...
			if (!free_sect_nr) {
				...
				for (; ((fsbuf[j] >> k) & 1) != 0; k++) {} //这里忽略了内存空洞的情况，直接以为1的条件循环到了第一个不为1的地方
