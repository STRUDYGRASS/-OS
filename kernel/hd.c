...
PRIVATE void	get_part_table		(int drive, int sect_nr, PARTITION_ENTRY * entry);
...
PRIVATE void partition(int device, int style)
{
	...
	PARTITION_ENTRY part_tbl[NR_SUB_PER_DRIVE];

	if (style == P_PRIMARY) {
		get_part_table(drive, drive, part_tbl); //主分区会按顺序排在这里，逻辑分区是嵌套读取
...
PRIVATE void get_part_table(int drive, int sect_nr, PARTITION_ENTRY * entry)
{
	...
	memcpy(entry,
	       ...
	       sizeof(PARTITION_ENTRY) * NR_PART_PER_DRIVE);
}
